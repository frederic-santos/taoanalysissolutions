...
\begin{exo}{2.2.5}{Prove the strong principle of induction, formulated
  ...
  First let's introduce a small lemma (similar to Proposition 2.2.12\textit{(e)}):
  for any natural number $a$ and $b$, $a<\successor{b}$ iff $a\leq
  b$. Indeed:
  \begin{itemize}
  \item If $a < \successor{b}$, then $\successor{b} = a+n$ for a given
    ...
    $a \leq b$.
  \item Conversely, if $a \leq b$, there exists a natural number $m$
  ...
  m_0$. This demonstrates the principle of string induction.
\end{exo}
\end{document}
