...
\begin{exo}{6.6.5}{Prove Proposition 6.6.6. (Hint: to show that (a)
  ...
  \item First we show that (b) implies (a). Let be $f : \nn
    ...
    $n \geq M \Rightarrow |a_{f(n)} - L| < \epsilon$. We know that $f$
  ...
  \item Now we prove that (a) implies (b).
    ...
    \item Now let be $f : \nn \rightarrow \nn$ the strictly increasing
      ...
      \begin{equation}
        ...
        \Rightarrow |a_{f(j)} - L| \leq \epsilon
      \end{equation}
...
\begin{exo}{7.1.2}{Prove Proposition 7.1.11.}
  ...
  elements, $f : X \rightarrow \rr$ a function, and $g$ a bijection
  ...
  \item If $X = \{x_0\}$, we have $\sum_{x \in X} f(x) = f(x_0)$.
    ...
    In this case, $X$ has $n = 1$ element, and $g : \{1\} \rightarrow
    X$ such that $g(1) = x_0$ is a bijection. Thus, by definition, we
    have $\sum_{x \in X} f(x) = \sum_{i=1}^{1} f(g(i)) = f(g(1)) =
    f(x_0)$.
  \item If $X$ is a finite set and $g : Y \rightarrow X$ a bijection,
    ...
    bijection $h : \intset{1}{n} \rightarrow X$ such that $\sum_{x \in
      X} f(x) = \sum_{i=1}^{n} f(h(i))$.

    On the other hand, let be $k : \intset{1}{n} \rightarrow Y$ a
  ...
  \item If $X, Y$ are sets such that $X \cap Y = \emptyset$, and $f :
    X \cup Y \rightarrow \rr$, we have $\sum_{z \in X \cup Y} f(z) =
    \sum_{x \in X} f(x) + \sum_{y \in Y} f(y)$.

    Suppose that $X$ has $n$ elements and $Y$ has $m$ elements. Thus,
    there exist two bijections $g: \intset{1}{n} \rightarrow X$ and
    $h: \intset{1}{m} \rightarrow Y$. Thus, by definition, we have
    ...
    \rightarrow X \cup Y$, defined by: $k(i) = g(i) \in X$ if $1 \leq i \leq
  ...
  \item If $X$ is a finite set, then we have $\sum_{x \in X} (f(x) +
    ...
    bijection $h : \intset{1}{n} \rightarrow X$. Then we have:
    \begin{align*}
  ...
  \item If $c$ is a real number, the we have $\sum_{x \in X} cf(x) = c
    ...
    bijection $\intset{1}{n} \rightarrow X$, we have:
  ...
  \item If $f,g$ are functions such that $f(x) \leq g(x)$ for all $i
    ...
    If $X$ has $n$ elements, let $h : \intset{1}{n} \rightarrow X$ be
  ...
  \item Triangle inequality: we have $\aval{\sum_{x \in X} f(x)} \leq
    ...
    If we consider the function $k : X \rightarrow \rr$ such that
    $k(x) = |f(x)|$ for all $x \in X$; and $h : \intset{1}{n}
    \rightarrow X$ a bijection, we have:
    \begin{align*}
...
\begin{exo}{7.1.5}{Let $X$ be a finite set, let $m$ be an integer, and
  ...
  \item Now suppose inductively that $\limit{\sum_{x \in X} a_n(x)} =
    ...
    Let be $h : \intset{1}{r+1} \rightarrow X$ a bijection. By
...
\begin{exo}{8.1.4}{Prove Proposition 8.1.8.}
  ...
  \item Now, let's prove that $f_{|A} : A \rightarrow f(\nn)$ is
    ...
    such that $f(a) = y$; i.e., $f_{|A} : A \rightarrow f(\nn)$ is
    surjective.
  \end{itemize}

  Thus, $f_{|A} : A \rightarrow f(\nn)$ is bijective. By Corollary
...
\begin{exo}{8.1.6}{Let $A$ be a set. Show that $A$ is at most
  ...
  \item If $A$ is finite, then $A$ has a finite number of
    ...
    bijective function $g : A \rightarrow \intset{1}{n}$. Now let be
    $f : A \rightarrow \nn$ such that $f(a) = g(a)$ for all $a \in A$.
    In particular, $f$ is injective---but not necessarily surjective.
  \end{enumerate}

  In both cases, we have found an injective map $f : A \rightarrow \nn$
  as required. This closes the proof.
\end{exo}
...
\begin{exo}{8.1.7}{Prove Proposition 8.1.10.}
  ...
  $f : \nn \rightarrow X$ and $g : \nn \rightarrow Y$. Let be
  $h : \nn \rightarrow X \cup Y$ a function defined by $h(2n) = f(n)$
...
\begin{exo}{8.1.8}{Use Corollary 8.1.13 to prove Corollary 8.1.14.}
  ...
  By definition, there exist two bijections $f : X \rightarrow \nn$
...
\begin{exo}{8.2.1}{Prove Lemma 8.2.3.}
  ...
  \item First suppose that
    ...
    Let be $N$ a natural number. Since $g$ is a bijection from $\nn$
    to $X$, the set $A_N := g(\intset{0}{N})$ is a (finite) subset of
    $X$ for any value of $N$. Furthermore, since the restriction
    $g : \intset{0}{N} \rightarrow A_N$ is bijective, we have
    $\sum_{x \in A_N} |f(x)| = \sum_{n=0}^N |f(g(n))|$. Thus, by our
    initial hypothesis,
    \begin{equation} \label{eq:821a}
    ...
    bounded (Proposition 6.3.8). By equation \eqref{eq:821a}, we have
    $\sup (S_N)_{N=0}^\infty < \infty$, which means that
    $(S_N)_{N=0}^\infty$ converges. It means that $\sum_{n=0}^{\infty}
    |f(g(n))|$ converges for some bijection $g : \nn \rightarrow X$,
    i.e. that $\sum_{x \in X} f(x)$ is absolutely convergent by
    Definition 8.2.1.
