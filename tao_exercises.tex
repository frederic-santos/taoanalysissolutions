...
\begin{exo}{7.2.5}{Prove Proposition 7.2.14.}
  ...
  \item The sequence of partial sums, $(S_N)_{N=m}^\infty$, converges
    iff $(S_N)_{N=m+k}^\infty$ is convergent (recall Exercise 6.1.3
    or Remark 6.1.9). Thus, if one of the two formal series
    $\sum_{n=m}^{\infty}a_n$ and $\sum_{n=m+k}^{\infty}a_n$ is
    convergent, the other one is also convergent.

    Furthermore, for all integers $N, k$, we know by Lemma 7.1.4(a)
    that we have $\sum_{n=m}^{N}a_n = \sum_{n=m}^{m+k-1}a_n +
    \sum_{n=m+k}^{N}a_n$.
