...
\bigskip
\begin{exo}{3.4.11}{Let $X$ be a set, let $I$ be a non-empty set, and
    for all $\alpha \in I$ let $A_\alpha$ be a subset of X. Show that $X \setminus \bigcup_{\alpha \in
      I} A_\alpha = \bigcap_{\alpha \in I} (X \setminus A_\alpha)$ and $X \setminus \bigcap_{\alpha \in
      I} A_\alpha = \bigcup_{\alpha \in I} (X \setminus A_\alpha)$.}

  We will just prove the first assertion here, since the second one
  can be proved in a very similar fashion.

  By definition, $x \in X \setminus \bigcup_{\alpha \in I} A_\alpha $ if and only if $x \in X$ and
  $x \notin \bigcup_{\alpha \in I} A_\alpha$. Equivalently, we have $x \in X$ and $x \notin A_\alpha$ for
  all $\alpha \in I$. We can just switch the last quantifier to say that
  ``for all $\alpha \in I$, we have $x \in X$ and $x \notin A_\alpha$''. In other words,
  we have $x \in \bigcap_{\alpha \in I} (X \setminus A_\alpha)$, as expected.
\end{exo}
...
\begin{exo}{4.3.1}{Prove Proposition 4.3.3.}
  ...
  \begin{enumerate}[label=\emph{(\alph*)}]
    ...
    \item Show that $d(x,z) \leq d(x,y) + d(y,z)$.
...
\begin{exo}{5.4.1}{Prove Proposition 5.4.4.}
  ...
  \item Next we show that at least one of the statements ``$x$ is
    ...
    N$. In such a case, we would have by triangle inequality:
    \[
...
\begin{exo}{6.1.4}{Let $\seq{a_n}{m}$ be a sequence of real numbers,
  ...
  \item First, suppose that $\seq{a_n}{m}$ converges to $c$. Let
    ...
    $M \geq m$ such that
    $n+k \geq M \implies |a_{n+k} - c| \leq \epsilon$. We know that
    there exists $N \geq m$ such that
    $n \geq N \implies |a_n -c| \leq \epsilon$. But for any
    $k \geq 0$, we have $n + k \geq n$, so that whenever $n \geq N$,
  ...
  \item Now prove the converse implication: suppose that
    ...
    $n \geq N \implies |a_n -c| \leq \epsilon$. We already know that there exists
    $M \geq m$ such that
    $n \geq M \implies |a_{n+k} - c| \leq \epsilon$. Let be
    $N := M+k$. We can see that if $n \geq N$, we have $n \geq M+k$,
    i.e. $n-k \geq M$, and thus $|a_{(n-k)+k} - c| \leq \epsilon$ as required. So,
    $N := M+k$ is suitable, and $\seq{a_n}{m}$ converges to $c$.
  \end{itemize}
\end{exo}
...
\begin{exo}{6.1.5}{Prove Proposition 6.1.12.}
  ...
  Now let be $j,k \geq N$ two natural numbers. We have, by triangle
  inequality:
  \begin{align*}
    |a_j - a_k| &= |a_j - c + c - a_k| \\
                & \leq |a_j - c| + |a_k - c| \\
...
\begin{exo}{6.1.8}{Prove Theorem 6.1.19 about limit laws.}
  ...
  \item Let be $\epsilon > 0$. We have to prove that there exists a
    ...
    Let be $N := \max(N_1, N_2)$. Thus, by triangle inequality, we
  ...
  \item Following Tao's hint, we begin by proving an intermediate
    ...
    ``second formula'' of triangle inequality, we have:
    \begin{equation}
      \label{eq:618e}
...
\begin{exo}{6.4.1}{Prove Proposition 6.4.5.}
  ...
  \item Now let's prove that $c$ is the \emph{only} limit point of
    ...
    integer $N \geq m$ such that ${|a_n - c| \leq \epsilon}$ for all
    $n \geq N$. Also, since $c'$ is a limit point, there exists
    $n_0 \geq N$ such that $|a_{n_0} - c'| \leq \epsilon$. Thus,

    \begin{align*}
      |c-c'| & \leq |c - a_{n_0} + a_{n_0} - c'| \\
             &\leq |a_{n_0} - c| + |a_{n_0} - c'| \\
             & \leq \epsilon + \epsilon =
               2\epsilon = \frac{2}{3} |c-c'|
    \end{align*}
...
\begin{exo}{6.4.3}{Prove parts (c), (d) and (e) of Proposition
  ...
  \item Prove that $\seq{a_n}{m}$ converges to $c$ iff $L^+ = L^- =
    ...
    \item Conversely, let's suppose that $\seq{a_n}{m}$ converges to
      ...
      We then derive, by triangle inequality:
...
\begin{exo}{7.1.1}{Prove Lemma 7.1.4. (Hint: you will need to use
  ...
  \begin{enumerate}[label=\emph{(\alph*)}]
  \item For all integers $m \leq n \leq p$, we have $\sum_{i=m}^{n} a_i +
    \sum_{i=n+1}^{p} a_i = \sum_{i=m}^{p} a_i$.

    Let's use induction on $p$, while keeping $m, n$ fixed. Since we
    have $n \leq p$, the base case is $p = n$. For $p=n$, we have:
    \[
      \sum_{i=m}^{n} a_i + \sum_{i=n+1}^{p} a_i
      = \sum_{i=m}^{p} a_i + \sum_{i=p+1}^{p} a_i
      = \sum_{i=m}^{p} a_i + 0 =
      \sum_{i=m}^{p} a_i,
    \]
    as expected. The base case is done. Now let's suppose inductively
    that
    $\sum_{i=m}^{n} a_i + \sum_{i=n+1}^{p} a_i = \sum_{i=m}^{p} a_i$, and let's
    prove that this equality still holds for $p+1$. We have:
    \begin{align*}
      \sum_{i=m}^{n} a_i + \sum_{i=n+1}^{p+1} a_i
      &= \sum_{i=m}^{n} a_i + \sum_{i=n+1}^{p} a_i + a_{p+1}
      &\text {(Definition 7.1.1)} \\
      &= \sum_{i=m}^p a_i + a_{p+1}
      &\text{(induction hypothesis)} \\
      &= \sum_{i=m}^{p+1} a_i &\text{ (Definition 7.1.1)}
    \end{align*}
    which closes the induction and proves the equality for all
    integers $m \leq n \leq p$.
  ...
  \item For $m \leq n$ two integers, we have $\sum_{i=m}^{n} (a_i+b_i)
    ...
    \begin{align*}
      ...
      &= \sum_{i=m}^{n} (a_i+b_i) + (a_{n+1} + b_{n+1}) \\
      &= \left(\sum_{i=m}^{n} a_i + \sum_{i=m}^{n} b_i\right) + a_{n+1} +
        b_{n+1}
      &\text{(induction hypothesis)} \\
  ...
  \item For $m \leq n$ integers, and $c$ a real number, we have
    ...
    \begin{align*}
      \sum_{i=m}^{n+1}ca_i
      &= \sum_{i=m}^{n}ca_i + ca_{n+1} \\
      &= c \left(\sum_{i=m}^{n}a_i\right) + ca_{n+1}
      &\text{(induction hypothesis)}\\
      &= c \left(\sum_{i=m}^{n}a_i + a_{n+1}\right)\\
      &= c \sum_{i=m}^{n+1} a_i
    \end{align*}
    which closes the induction. The property is thus true for all $n
    \geq m$.
  \item For $m \leq n$ integers, prove that $|\sum_{i=m}^{n}a_i| \leq
    ...
    \begin{align*}
      \aval{\sum_{i=m}^{n+1}a_i}
      &:= \aval{\sum_{i=m}^{n} a_i + a_{n+1}} \\
      &\leq \aval{\sum_{i=m}^{n} a_i} + |a_{n+1}|
      &\text{(Proposition 4.3.3(b))} \\
      & \leq \sum_{i=m}^{n} |a_i| + |a_{n+1}|
      &\text{(induction hypothesis)} \\
  ...
  \item For $m \leq n$ integers, prove that, if $a_i \leq b_i$ for all
    $m \leq i \leq n$, we have $\sum_{i=m}^{n} a_i \leq \sum_{i=m}^{n} b_i$.
    ...
    Now let's suppose inductively that we have
    $\sum_{i=m}^{n} a_i \leq \sum_{i=m}^{n} b_i$. Then, if $a_{n+1} \leq
    b_{n+1}$, we have:
    ...
    \begin{align*}
      ...
      & \leq \sum_{i=m}^{n} b_i + b_{n+1} &\text{(induction hypothesis)} \\
...
\begin{exo}{7.1.2}{Prove Proposition 7.1.11.}
  ...
  \item If $X, Y$ are sets such that $X \cap Y = \emptyset$, and $f :
    ...
    Thus, we have:
    \begin{align*}
      \sum_{x \in X \cup Y} f(x)
      &= \sum_{i=1}^{n+m}f(k(i)) \\
      &= \sum_{i=1}^{n}f(k(i)) + \sum_{i=n+1}^{n+m}f(k(i))
      &\text{(by Lemma 7.1.4(a))} \\
      &= \sum_{i=1}^{n}f(g(i)) + \sum_{i=n+1}^{n+m}f(h(i)) \\
      &= \sum_{x \in X} f(x) + \sum_{y \in Y} f(y)
    \end{align*}
  ...
  \item If $c$ is a real number, the we have $\sum_{x \in X} cf(x) = c
    ...
    \begin{align*}
      \sum_{x \in X} cf(x)
      &= \sum_{x \in X} g(x) = \sum_{i=1}^{n} g(h(i)) = \sum_{i=1}^n cf(h(i)) \\
      &= c \times \sum_{i=1}^{n} f(h(i)) &\text{(Lemma 7.1.4(d))} \\
  ...
  \item If $f,g$ are functions such that $f(x) \leq g(x)$ for all $i
    ...
    \begin{align*}
      \sum_{x \in X} f(x)
      &= \sum_{i=1}^{n} f(h(i)) \\
      & \leq \sum_{i=1}^{n} g(h(i)) &\text{(Lemma 7.1.4(f))} \\
      &= \sum_{x \in X} g(x)
    \end{align*}
  \item Triangle inequality: we have $\aval{\sum_{x \in X} f(x)} \leq
    ...
    \begin{align*}
      \aval{\sum_{x \in X} f(x)}
      &= \aval{\sum_{i=1}^{n} f(h(i))} \\
      & \leq \sum_{i=1}^{n} |f(h(i))| &\text{ (Lemma 7.1.4(e))} \\
      & = \sum_{i=1}^{n} k(h(i)) = \sum_{x \in X} k(x) \\
      &= \sum_{x \in X} |f(x)|
    \end{align*}
  \end{enumerate}
\end{exo}

\begin{exo}{7.1.3}{Form a definition for the finite products
    $\prod_{i=m}^n a_i$ and $\prod_{x \in X} f(x)$. Which of the above results
    for finite series have analogues for finite products?}

  By mimicking Definitions 7.1.1 and 7.1.6, we can form the following
  definitions for finite products $\prod_{i=m}^n a_i$, and products over
  finite sets $\prod_{x \in X} f(x)$ (where $X$ is finite).
  
  \begin{enumerate}[label=(\roman*)]
  \item Let $m,n$ be integers, and let $(a_i)_{i=m}^n$ be a finite
    sequence of real numbers. We define the finite product
    $\prod_{i=m}^n a_i$ recursively by:
    \begin{align*}
      \prod_{i=m}^n a_i &:= 1
      &\text{ whenever } n < m \\
      \prod_{i=m}^{n+1} a_i &:= \left(\prod_{i=m}^n a_i\right) \times a_{n+1}
      &\text{ whenever } n \geq m-1.
    \end{align*}
    
  \item Let $X$ be a finite set of $n$ elements, $f : X \to \rr$ be
    function, and $g$ a bijection from
    $\{i \in \nn : 1 \leq i \leq n\}$ to $X$. We thus define
    \[
      \prod_{x \in X} f(x) := \prod_{i=1}^n f(g(i)).
    \]
  \end{enumerate}
  With the above definitions, we can form analogues for Lemma 7.1.4
  and Proposition 7.1.11. Almost all their statements can be adapted
  (and then remain true) for finite products; Lemma 7.1.4(f) and
  Proposition 7.1.11(h) are the only statements that do not remain
  true if we replace the sign ``sum'' by a sign ``product''. However,
  if all the $a_i$ are positive, then an analogue for these two
  statements is now possible (why? See Proposition 5.4.7(e)).
\end{exo}
\pagebreak
...
\begin{exo}{7.1.4}{Define the factorial function $n!$ for natural
  ...
  \begin{align*}
    \frac{n!}{(j-1)! (n-j+1)!} +
    \frac{n!}{j! (n-j)!}
    &= \frac{n! \times j}{j! (n-j+1)!} + \frac{n! \times (n-j+1)}{j! (n-j+1)!} \\
    &= \frac{n! \times (n+1)}{j! (n-j+1)!} \\
    &= \frac{(n+1)!}{j! (n+1-j)!}
  \end{align*}

  Now we can go back to the main proof, for which we will induct on
  $n$.
  \begin{itemize}
  \item Let's start with the base case $n=0$. On the one hand, by
    Definition 5.6.1, we have $(x+y)^0 = 1$. On the other hand, we
    have
    $\sum_{j=0}^{0} \frac{n!}{j!(n - j)!} x^{j}y^{n-j} = \frac{0!}{0! \times
      0!} x^0 y^0 = 1$, so that the base case is done.
...
\begin{exo}{7.1.5}{Let $X$ be a finite set, let $m$ be an integer, and
  ...
  \item Now suppose inductively that $\limit{\sum_{x \in X} a_n(x)} =
    ...
    \begin{align*}
      \limit{\sum_{x \in X} a_n(x)}
      &= \limit{\left( \sum_{i=1}^r a_n(h(i)) + a_n(h(r+1)) \right)} \\
      &= \limit{\sum_{i=1}^r a_n(h(i))} + \limit{a_n(h(r+1))} \\
      &= \sum_{i=1}^r \limit{a_n(h(i))} + \limit{a_n(h(r+1))}
      &\text{(induction hypothesis)} \\
      &= \sum_{i=1}^{r+1} \limit{a_n(h(i))}
      &\text{(Definition 7.1.1)} \\
      &= \sum_{x \in X} \limit{a_n(x)} \text{(Proposition 7.1.11(c))}
    \end{align*}
    so that the property is also true when $X$ has $r+1$ elements.
    This closes the induction.
  \end{itemize}
\end{exo}

\begin{exo}{7.2.1}{Is the series $\sum_{n=1}^\infty (-1)^n$ convergent or
    divergent? Can you now resolve the difficulty in Example 1.2.2?}

  Consider the partial sums defined by $S_N := \sum_{n=1}^{N} (-1)^n$. We
  note that ${S_1 = -1}$, $S_2 = 0$, $S_3 = -1$, etc. More generally,
  a quick induction shows that $S_{2K-1} = -1$ et $S_{2K} = 0$ for all
  $K \geq 0$.
  
  It is thus clear that the limit superior $L^+$ of
  $(S_N)_{N=1}^\infty$ is equal to $0$, and that its limit inferior
  $L^-$ is equal to $-1$ (why?). Thus, since $L^+ \neq L^-$, the sequence
  of partial sums $(S_N)_{N=1}^\infty$ diverges (Proposition 6.4.12(f)),
  and the series $\sum_{n=1}^{\infty} a_n$ is divergent.

  Example 1.2.2 is not valid because the series presented there does
  not converge: the ``sum'' $S$ is not a real number, and thus the
  usual algebraic laws do not apply.
\end{exo}
\bigskip
...
\begin{exo}{7.2.4}{Prove Proposition 7.2.9.}

   Let $\sum_{n=m}^{\infty} a_n$ be a convergent formal series; i.e., the
   formal series $\sum_{n=m}^{\infty} |a_n|$ is convergent. Let be $\epsilon >
   0$. Thus, by Proposition 7.2.5, there exists an $N \geq m$ such that
   $\aval{\sum_{n=p}^{q} |a_n|} \leq \epsilon$ for all $p,q \geq N$. But by Lemma
   7.1.4(e), we have
    \[
      \aval{\sum_{n=p}^{q} a_n}
      \leq \sum_{n=p}^{q} |a_n|
      ...
      \leq \epsilon,
    \]
    which means (once again by Proposition 7.2.5) that the formal
    series $\sum_{n=m}^{\infty} a_n$ converges.

    \medskip
    Now we define, for all $N \geq m$, the partial sums
    $S_N := \sum_{n=1}^{N} a_n$ and $T_N := \sum_{n=1}^{N} |a_n|$. By Lemma
    7.1.4(e), we have
    \begin{equation}
      \label{eq:7.2.4a}
      |S_N| \leq T_N \text{ for all } N \geq m.
    \end{equation}
    Let's take the limit as $N \to \infty$ in inequality \eqref{eq:7.2.4a}
    (which is legit since we know that both $(S_N)$ and $(T_N)$
    converge) :
    \begin{itemize}
    \item on the one hand, we have
      $\lim_{N \to \infty}{T_N} = \sum_{n=m}^{\infty} |a_n|$ (by Definition 7.2.2) ;
    \item on the other hand, we have
      $\lim_{N \to \infty}{S_N} = \sum_{n=m}^{\infty} a_n$, and since the function
      $f : x \mapsto |x|$ is continuous on $\rr$, we have
      \[
        \lim_{N \to \infty}{|S_N|} = \aval{\lim_{N \to \infty}{S_N}}
        = \aval{\sum_{n=m}^{\infty} a_n}
      \]
      by Proposition 9.4.7(b).
    \end{itemize}
    We can thus conclude
    \[
      \aval{\sum_{n=m}^{\infty} a_n} \leq \sum_{n=m}^{\infty} |a_n|,
    \]
    as expected.
\end{exo}
\bigskip
...
\begin{exo}{7.3.2}{Prove Lemma 7.3.3.}
  ...
  \item If $|x| < 1$, then let's prove that for all $N \geq 0$, the
    partial sum $S_N := \sum_{n=0}^{N}x^n$ is equal to
    $(1-x^{N+1})/(1-x)$. Let's induct on $N$, starting with the base
    case $N=0$. In this case, we have $S_0 = \sum_{n=0}^{0}x^n = 1$; and
...
\begin{exo}{7.3.3}{Let $\sum_{n=0}^{\infty} a_n$ be an absolutely convergent
    series of real numbers such that $\sum_{n=0}^{\infty} |a_n| = 0$. Show
    that $a_n = 0$ for every natural number $n$.}

  Suppose for the sake of contradiction that there exists a anturel
  number $m$ such that $a_m \neq 0$. Thus, in particular,
  $|a_m| > 0$, and for all $N \geq m$, we have
  \[
    \sum_{n=0}^{N} |a_n| \geq |a_m| > 0.
  \]
  We can take the limit as $N \to \infty$ in this inequality leads to
  $\sum_{n=0}^{\infty} |a_n| \geq |a_m| > 0$, a contradiction with the initial
  hypothesis.
\end{exo}
...
\begin{exo}{9.1.6}{Let $X$ be a subset of $\rr$. Show that $\adh{X}$
  ...
  \epsilon/2$. Thus, by triangle inequality, we have $|x'' - x| \leq
...
\begin{exo}{10.1.2}{Prove Proposition 10.1.7.}
  ...
  \begin{itemize}
  \item First we show that (a) $\implies$ (b). If $f$ is
    differentiable at $x_0$, there exists a real number $L$ such that
    ${\lim_{x \to x_0 ; \, x \in X \setminus \{x_0\}} \frac{f(x) - f(x_0)}{x -
        x_0} = L}$. Let be an arbitrary $\epsilon > 0$. There exists
    (Definition 9.3.6) a real number $\delta > 0$ such that, for all
    $x \in X \setminus \{x_0\}$ with $|x-x_0| \leq \delta$, we have
    \[
      \left| \frac{f(x) - f(x_0)}{x - x_0} - L \right| \leq \epsilon,
    \]
    and then, by multiplying by $|x-x_0|$ on both sides,
    \begin{equation}
      \label{eq:10.1.2a}
      \left| f(x) - f(x_0) - L(x-x_0) \right| \leq \epsilon|x-x_0|.
    \end{equation}
    Furthermore, \eqref{eq:10.1.2a} remains true when $x=x_0$, since
    both sides are then null. This means that (b) is true, as
    required.
  \item Now we show that (b) $\implies$ (a). Let be an arbitrary
    $\epsilon > 0$. Since (b) is true, there exists a real number
    $\delta > 0$ such that, for all $x \in X$ with
    $|x-x_0| \leq \delta$, we have
    $\left| f(x) - f(x_0) - L(x-x_0) \right| \leq \epsilon|x-x_0|$. Of course,
    since this applies when $x \in X$, it also applies when
    $x \in X \setminus \{x_0\}$. Thus, for all
    $x \in X \setminus \{x_0\}$ (note in particular that this implies
    $|x-x_0| \neq 0$), we have
    \[
      \left| f(x) - f(x_0) - L(x-x_0) \right| \leq \epsilon|x-x_0|,
    \]
    and then, after dividing by $|x - x_0|$ on both sides,
    \[
      \left| \frac{f(x) - f(x_0)}{x - x_0} - L \right| \leq \epsilon.
    \]
    Thus, it means that
    ${\lim_{x \to x_0 ; \, x \in X \setminus \{x_0\}} \frac{f(x) - f(x_0)}{x -
        x_0} = L}$, i.e. that (a) is true.
  \end{itemize}
  This shows that (a) $\iff$ (b), as expected.
\end{exo}
...
\begin{exo}{10.1.3}{Prove Proposition 10.1.10.}
  ...
  $f : X \to \rr$ a function which is differentiable at $x_0 \in X$. It
  means that we have
  \begin{equation}
    \label{eq:10.1.3a}
...
\begin{exo}{10.1.7}{Prove Theorem 10.1.5.}
  ...
  \begin{equation}
    ...
    \epsilon|x-x_0|
  \end{equation}
  and, using the triangle inequality, this also implies that
   \begin{equation}
  ...
  By combining all these facts, we want to show that we have the
  ...
  $\epsilon \geq \epsilon'(\epsilon' + |L_1| + |L_2|)$ --- the
...
\begin{exo}{10.3.5}{Give an example of a subset $X \in \rr$ and a
    ...
    diﬀerent from those in Proposition 10.3.3. What is the diﬀerence,
    and how can one exploit that diﬀerence to obtain the example?)}
...
\begin{exo}{11.1.2}{Prove Corollary 11.1.6.}
  ...
  \begin{itemize}
  \item Since $I$ is bounded, we have $I \subseteq [-M, M]$ for some
    $M>0$. Similarly, we have $J \subseteq [-N, N]$ for some $N>0$.
    Thus, if we set $P := \max(M, N)$, we have clearly $I \cap J
    \subseteq [-P, P]$, so that $I \cap J$ is bounded.
  \item Let be $x<y$ in $I \cap J$, and another element $z$ such that
    $x<z<y$. Let's show that $z \in I \cap J$. Since $x,y \in I \cap J$,
    then in particular $x,y \in I$, which is a connected set. Thus,
...
\begin{exo}{11.2.2}{Prove Lemma 11.2.8. (Hint: use Lemmas 11.1.18 and
  ...
  constant. Indeed, for any $K \in \P \# \P'$, we have $f|_K = c_K$
  and $g|_K = d_K$, so that $(f+g)|_K = c_K + d_K$. A similar argument
...
\begin{exo}{11.2.3}{Prove Proposition 11.2.13.}
  ...
  \item Let's begin with a preliminary result. Consider $\P$ and $\P'$
    ...
    is trivial). In particular, $x \in I$. Since $\P'$ is a partition of
    $I$, there exists exactly one $K \in P'$ such that $x \in K$.  Thus,
    $x \in J \cap K$, and since $K$ is unique, so is $J \cap K$.  Thus,
    $S_J$ is indeed a partition of $J$.
  \item Another preliminary result : as a consequence, for any
    $J \in \P$, we have:
    \begin{align*}
      \sum_{K \in \P'} |J \cap K|
      &= \sum_{X \in S_J} |X| &\text{(Proposition 7.1.11(c))} \\
      &= |J| &\text{(Theorem 11.1.13)}
    \end{align*}
...
\begin{exo}{11.2.4}{Prove Theorem 11.2.16.}
  ...
  Let be $f : I \to \rr$ a piecewise constant function on a given
  partition $\P'$ of $I$, and $g : I \to \rr$ a piecewise constant
  function on a given partition $\P'$ of $I$. Then, $f$ and $g$ are
  piecewise constant on $\P := \P' \# \P''$ (by Lemma 11.1.18 and
  11.2.7), and $p.c. \int_I f = p.c. \int_{[\P]} f$ and
  $p.c. \int_I g = p.c. \int_{[\P]} g$.
  ...
  \item Show that $p.c. \int_I (f+g) = p.c. \int_I f + p.c. \int_I g$.
    ...
    \begin{equation*}
      ...
      \sum_{J \in \P} c_J |J| + \sum_{J \in \P} d_J |J|
      = \sum_{J \in \P} (c_J + d_J) |J|.
    \end{equation*}
...
\begin{exo}{11.3.5}{Prove Proposition 11.3.12.}
  ...
  \item First, let's prove that
    ...
    constant with respect to $\P$ that majorizes $f$ on $I$. Thus, by
    Lemma 11.3.11 and by definition of an infimum, we have
    \[
      p.c. \int_I g \geq \mathbf{U}(f, \P) \geq \inf \{\mathbf{U}(f, \P) : \P
      \text{ partition of } I\}.
    \]

    Since this is true for an arbitrary function $g$, we see that the
    real number
    $\inf \{\mathbf{U}(f, \P) : \P \text{ partition of } I\}$ is a
    lower bound for the set $\{p.c. \int_I g : g \text{ p.c. on $I$
      and majorizes } f\}$.

    By definition of an infimum, we thus have
    \begin{align*}
      \supint{f}{I}
      &:= \inf \left\{p.c. \int_I g : g \text{ p.c. on $I$ and majorizes } f\right\} \\
      &\geq \inf \{\mathbf{U}(f, \P) : \P \text{ partition of } I\},
    \end{align*}
    as expected.
...
\begin{exo}{11.4.1}{Prove Theorem 11.4.1.}
  ...
  \item If we suppose that $f|_J$ and $f|_K$ are Riemann integrable,
    ...
    distant of $2\epsilon$ of each other\footnote{Formally, if we have
      ...
      $0 \leq z-y \leq 2 \epsilon$ (why? Use triangle
...
\bigskip
\begin{exo}{11.6.3}{Prove Proposition 11.6.4.}

  Let be $f : [0, +\infty[ \to \rr$ a positive, decreasing function.
  We have to show that $\sum_{n=0}^\infty f(n)$ converges iff
  $\sup_{N>0} \int_{[0,N]}f$ is finite.

  First observe that since $f$ is decreasing and positive, it is
  bounded, and is thus Riemann-integrable on any interval of the form
  $[0,N]$ by Corollary 11.6.3.

  Also, we have $f(n) \geq f(x) \geq f(n+1)$ for all $x$ in an
  interval $[n, n+1[$. This motivates the following argument:
  
  \begin{itemize}
  \item Let be $\P := \{[n, n+1[ \, : \, n \in \nn\}$ a partition of
    $[0, +\infty[$.
  \item Let be the constant piecewise function
    $g : [0,+\infty[ \to \rr$ defined by $g(x) := f(n)$ when
    $x \in [n, n+1[$. The function $g$ clearly majorizes $f$ on
    $[0, +\infty[$.
  \item Let be the constant piecewise function
    $h : [0,+\infty[ \to \rr$ defined by $h(x) := f(n+1)$ when
    $x \in [n, n+1[$. The function $g$ clearly minorizes $f$ on
    $[0, +\infty[$.
  \item Thus, on any interval $[n, n+1[$ with $n \in \nn$, we have:
    \[ f(n) = \int_{[n,n+1[} g(x) \geq \int_{[n,n+1[} f \geq
      \int_{[n,n+1[} h = f(n+1) \]
    We can use Theorem 11.4.1(g) to sum over such intervals, to get:
    \[ \sum_{n=0}^{N-1} f(n) \geq \int_{[0,N[} f \geq
      \sum_{n=0}^{N-1} f(n+1)\]
    i.e., using Remark 11.4.2, and Lemma 7.1.4, 
    \begin{equation}
      \label{eq:11.6.3}
      \sum_{n=0}^{N-1} f(n) \geq \int_{[0,N]} f \geq \sum_{n=1}^{N} f(n)
    \end{equation}
  \item In \eqref{eq:11.6.3}, if $\sum_{n=0}^\infty f(n)$ converges,
    then (since is equal to $\sup_N \sum_{n=0}^{N-1} f(n)$) this sum
    is an upper bound for $\int_{[0,N]} f$, so that the supremum
    $\sup_{n>0} \int_{[0,N]} f$ is necessarily finite.
  \item Conversely, if the supremum $\sup_{N>\int_{[0,N]} f } \int_{[0,N]} f$ is
    finite, then it is a (finite) upper bound for the $\sum_{n=1}^{N}
    f(n)$. Thus, the series $\sum_{n=0}^{\infty} f(n)$ converges, by
    Proposition 7.3.1.
  \end{itemize}
\end{exo}

\begin{exo}{11.6.4}{Give examples to show that both directions of the
    integral test break down if $f$ is not assumed to be monotone
    decreasing.}

  Consider the following two examples\footnote{Note that we must still
    choose piecewise constant functions here, since we still do not
    know how to compute the integral of a more general function.}:
  \begin{itemize}
  \item Let be the positive function $f: [0,+\infty[ \to \rr$ defined
    by:
    \[
      \begin{cases}
        f(x) = 0 &\text{ if } x \in [n, n+1/4], n \in \nn\\
        f(x) = 1 &\text{ if } x \in [n+1/4, n+3/4], n \in \nn\\
        f(x) = 0 &\text{ if } x \in [n+3/4, n+1], n \in \nn\\
      \end{cases}
    \]
    In particular, we have $f(n) = 0$ for all $n \in \nn$; and
    $\int_{[n, n+1]} f = 1/2$ for all $n \in \nn$. Thus, we clearly
    have $\sum_{n=0}^\infty f(n) = 0$, i.e. the series converges; but
    $\sup_{N > 0} \int_{[0,N]} f = \sup_{N>0} N/2 = +\infty$.
  \item Let be the positive function $g : [0, +\infty[ \to \rr$
    defined by:
    \[
      \begin{cases}
        f(x) = 1&\text{ if } x \in \nn\\
      ...
      \end{cases}
    \]
    For all $n \in \nn$, we clearly have $\int_{[n, n+1]} f = 0$, and
    thus $\int_{[0,N]} f = 0$. Consequently, we also have
    $\sup_{N>0} \int_{[0,N]} f = 0$.

    On the other hand, we have $\sum_{n=0}^N f(n) = N$, so that
    $\sum_{n=0}^\infty f(n) = \infty$. Thus, the series diverges, even
    though the sup over the integrals $\int_{[0,N]} f$ is finite.
  \end{itemize}
\end{exo}

\begin{exo}{11.8.1}{Prove Lemma 11.8.4. (Hint: modify the proof of
    Theorem 11.1.13.)}

  Let be $X$ a closed interval, $I \subseteq X$ a closed interval, and
  $\alpha : X \to \rr$ a function which is either monotone increasing
  or continuous, and $\P$ a partition of $I$. We have to show that
  \begin{equation}
    \label{eq:11.8.1}
    \sum_{J \in \P} \alpha[J] = \alpha[I]
  \end{equation}

  As in the proof of Theorem 11.1.13, we use induction on
  $n := \# \P$.
  \begin{itemize}
  \item The base case $n=0$ is trivial because in this case, $\P$ is
    empty, and thus $I$ must be itself the empty set. In such a case,
    both sides of \eqref{eq:11.8.1} are equal to $0$.

    Similarly, if $n=1$, then $I$ must be a singleton $\{a\}$. In such
    a case, both sides of \eqref{eq:11.8.1} are equal to
    $\alpha[\{a\}]$, so that the property is once again trivial.
  \item Now suppose inductively that the property is true when $\# \P
    =n$, and let's prove that it is still true when $\# \P = n+1$.

    First, note that once again, the property is trivial when $I =
    \emptyset$ (since all elements of $\P$ are then the empty set
    themselves), and when $I = \{a\}$ (since they can only be $\{a\}$
    or the empty set). Thus, we now consider that $I$ is of the form
    $[a,b]$, $]a,b]$, $[a,b[$ or $]a,b[$ (with $a<b$ two real
    numbers).{\pagebreak}

    Now we split into two sub-cases:
    \begin{enumerate}
    \item If $b \in I$, then $I$ is either $[a,b]$ or $]a,b]$.
      Furthermore, since $b \in I$, there exists exactly one
      $K \in \P$ such that $b \in K$. This interval $K$ must be either
      $[c,b]$, $]c,b]$ or $\{b\}$, with $a \leq c \leq b$.

      To avoid splitting the proof into an very deep imbrication of
      sub-cases, let's consider that $I = [a,b]$ and $K = [c,b]$, but
      the proof is very similar in the other cases. Thus, $I-K =
      [a,c[$. We thus have:
      \begin{align*}
        \alpha[I] &:= \Bigl(\lim_{x\to a^+} \alpha(x) -
                    \alpha(a)\Bigr) +
                    \Bigl(\lim_{x\to b^-} \alpha(x) - \lim_{x\to a^+}
                    \alpha(x) \Bigr) +
                    \Bigl(\alpha(b) - \lim_{x\to b^-} \alpha(x)\Bigr)
        \\
                  &= \alpha(b) - \alpha(a)\\
        \alpha[I-K] &:= \Bigl(\lim_{x\to a^+} \alpha(x) -
                      \alpha(a)\Bigr)
                      + \Bigl(\lim_{x\to c^-} \alpha(x) - \lim_{x\to a^+}
                      \alpha(x) \Bigr)\\
                  &= \lim_{x\to c^-} \alpha(x) - \alpha(a) \\
        \alpha[K] &:= \Bigl(\lim_{x\to c^+} \alpha(x) - \lim_{x\to
                    c^-} \alpha(x)\Bigr)
                    + \Bigl( \lim_{x\to b^-} \alpha(x) - \lim_{x\to
                    c^+} \alpha(x) \Bigr)
                    + \Bigl(\alpha(b) - \lim_{x\to b^-}
                    \alpha(x)\Bigr) \\
                  &= -\lim_{x \to c^-} \alpha(x) + \alpha(b)
      \end{align*}
      when $\alpha$ is supposed to be monotone, or more directly, using
      ...
      \end{align*}
      when $\alpha$ is supposed to be continuous and not necessarily
      monotone.

      In both cases, we can see that we have
      \begin{equation}
        \label{eq:11.8.1b}
        \alpha[I] = \alpha[I-K] + \alpha[K].
      \end{equation}
      And since $\P - \{K\}$ is a partition of $I-K$ of cardinality
      $n$, the induction hypothesis applies, and we get
      \[\alpha[I] = \alpha[K] + \alpha[I-K] = \alpha[K] + \sum_{J \in
          \P - \{K\}} \alpha[J] = \sum_{J \in \P} \alpha[J]\]
      as expected. This closes the induction.
    \item If $b \notin I$, then $I = [a,b[$ or $]a,b[$, and thus there
      exists exactly one $K \in \P$ which is equal to $]c,b[$ or
      $[c,b[$. We just have to follow the same approach as in the
      previous case.
    \end{enumerate}
  \end{itemize}
\end{exo}

\begin{exo}{11.8.2}{State and prove a version of Proposition 11.2.13
    for the Riemann-Stieltjes integral.}

  This proposition could be stated as follows:

  Let be $I$ a bounded interval, and $\P, \P'$ be partitions of $I$.
  Let be $f : I \to \rr$ a function which is piecewise constant with
  respect to $\P$ and $\P'$, and let be $\alpha : I \to \rr$ a
  continuous or increasing function. Then we have
  $p.c. \int_{[\P]} f \, d\alpha = p.c. \int_{[\P']} f \, d\alpha =:
  \int_I f \, d\alpha$.

  To prove this statement, we just have to modify Exercise 11.2.13 (we
  will reuse here the notations are results introduced in this former
  exercise).

  First let's consider $\P \# \P'$, which is a partition of $I$ finer
  than both $\P$ and $\P'$ (Lemma~11.1.8). Furthermore, $f$ is
  piecewise constant on $\P \# \P'$ (Lemma~11.2.7), so that
  $p.c. \int_{[\P \# \P']} f \, d\alpha$ is well-defined.

  We thus have:
  \begin{align*}
    p.c. \int_{[\P \# \P']} f \, d\alpha
    &:= \sum_{J \in \P \# \P'} c_J \, \alpha[J]
    &\text{ (Definition 11.8.5)}\\
    &= \sum_{(K,L) \in \P \times \P'} c_{K \cap L} \, \alpha[K \cap L] &
    \text{ (Definition 11.1.6)}\\
    &= \sum_{K \in \P} \sum_{L \in \P'} c_{K \cap L} \, \alpha[K \cap L]
    &\text{ (Corollary 7.1.14)} \\
    &= \sum_{K \in \P} c_K \Bigl( \sum_{L \in \P'} \alpha[K \cap L]\Bigr)
    & \text{ (because $c_K = c_{K \cap L}$)}\\
    &= \sum_{K \in \P} c_K (\alpha[K])
    &\text{ (Lemma 11.8.4)}\\
    &=: p.c. \int_{[\P]} f \, d\alpha&
  \end{align*}

  The same argument would show that $p.c. \int_{[\P \# \P']} f \,
  d\alpha = p.c. \int_{[\P']} f \, d\alpha$, which closes the proof.  
\end{exo}
\bigskip

\begin{exo}{11.8.3}{State and prove a version of Theorem 11.2.16 for
    the Riemann-Stieltjes integral.}

  This Theorem could be stated as follows.

  Let be $I$ a bounded interval, and $X$ a closed interval that
  contains $I$. Let be $f,g : I \to \rr$ two functions which are
  piecewise constant on $I$, and $\alpha : X \to \rr$ an
  \emph{increasing} function (note that in this paragraph, $\alpha$ is
  really supposed to be increasing, and not ``either continuous or
  increasing'' as before). Then we have:
  \begin{enumerate}[label=(\alph*)]
  \item
    $p.c. \int_I (f+g) \, d\alpha = p.c. \int_I f \, d\alpha + p.c.
    \int_I g \, d\alpha$.
  \item For all $c>0$, $p.c. \int_I (cf) \, d\alpha = c (p.c. \int_I
    f \, d\alpha)$.
  \item $p.c. \int_I (f-g) \, d\alpha = p.c. \int_I f \, d\alpha -
    p.c. \int_I g \, d\alpha$.
  \item If $f(x) \geq 0$ for all $x \in I$, then $p.c. \int_I f \,
    d\alpha \geq 0$.
  \item If $f(x) \geq g(x)$ for all $x \in I$, then $p.c. \int_I f \,
    d\alpha \geq p.c. \int_I g \, d\alpha$.
  \item If $f$ is the constant function $f(x) = c$, then $p.c. \int_I
    f \, d\alpha = c \alpha[I]$.
  \item Let be $J$ a closed interval such that $I \subseteq J$. Then
    the function $F : J \to \rr$ defined by
    \[
      \begin{cases}
        F(x) = f(x) & \text{ if } x \in I \\
        F(x) = 0 & \text{ if } x \in J - I
      \end{cases}
    \]
    is piecewise constant on $J$, and $p.c. \int_J F \, d\alpha = p.c.
    \int_I f \, d\alpha$.
  \item If $\{J,K\}$ is a partition of $I$ into two intervals $J$ and
    $K$, then the functions $f|_J : J \to \rr$ and $f|_K : K \to \rr$
    are piecewise constant on $J$ and $K$ respectively, and we have
    \[
      p.c.\int_I f \, d\alpha = p.c.\int_J f|_J \, d\alpha + p.c.
      \int_K f|_K \, d\alpha
    \]
  \end{enumerate}

  Now we give a proof for each statement above. Just before that, we
  introduce the general notation that $\P$ is a partition of $I$, and
  for all $J \in \P$, $c_J$ and $d_J$ are the constant values on $J$
  of $f$ and $g$ respectively.
  
  \begin{enumerate}[label=(\alph*)]
  \item Let be $\P$ of partition of $I$ such that $f$ and $g$ are both
    piecewise constant with respect to $\P$. (This partition certainly
    exists; if $f$ is piecewise constant w.r.t. $\P'$ and $g$ is
    piecewise constant w.r.t. $\P''$, we just have to take
    $\P := \P' \# \P''$.) Note that $f+g$ is piecewise constant on
    $\P$, and its constant value on each $J \in \P$ is $c_J + d_J$
    (Lemma~11.2.8). Thus we have
    \begin{align*}
      \int_I (f+g) \, d\alpha
      &:= \sum_{J \in \P} (c_J + d_J) \alpha[J] \\
      &= \sum_{J \in \P} c_J \, \alpha[J] + \sum_{J \in \P} d_J \, \alpha[J]\\
      &= \int_I f \, d\alpha + \int_I g \, d\alpha
    \end{align*}
    as expected (using the linearity of finite sums).
  \item The function $(cf)$ is piecewise constant with respect to
    $\P$, and on each $J \in \P$, its constant value is $cc_J$. Still
    using the linearity of finite sums, we thus have
    \begin{equation*}
      \int_I (cf) \, d\alpha := \sum_{J \in \P} cc_J \, \alpha[J]
      = c \sum_{J \in \P} c_J \alpha[J] = c \int_I f \, d\alpha
    \end{equation*}
    as expected.
  \item Direct consequence of (a) and (b) with $c = -1$.
  \item If $f(x) \geq 0$ for all $x \in I$, then $c_J \geq 0$ for all
    $J \in \P$. Thus,
    $p.c. \int_I f \, d\alpha = \sum_{J \in \P} c_J \, \alpha[J]$ is a
    finite sum of non-negative terms ($\alpha[J]$ is also non-negative
    because $\alpha$ is supposed to be increasing), and is thus
    non-negative itself.
  \item Direct consequence of (c) and (d).
  \item If $f(x) = c$ for all $x \in I$, then $f$ is piecewise
    constant with $c_J = c$ for all $J \in \P$. Thus we have
    \[p.c. \int_I f \, d\alpha := \sum_{J \in \P} c_J \, \alpha[J]
      = c \sum_{J \in \P} \alpha[J] = c \alpha[I]\]
    as expected.
  \item As shown in Exercise 11.2.4(g), there exists a partition
    $\P''$ of $J$ such that $\P'' = \P \cup \P'$, where $\P$ is a
    partition of $I$ and $\P'$ is a partition of $J-I$. Furthermore,
    $F$ is piecewise constant with respect to $\P''$, and is constant
    equal to $0$ on $\P'$.
    ...
    \begin{align*}
      p.c. \int_J F \, d\alpha
      &:= p.c. \int_{[\P'']} F \, d\alpha \\
      &= p.c. \int_{[\P \cup \P']} F \, d\alpha \\
      &= \sum_{K \in \P \cup \P'} c_K \, \alpha[K] \\
      &= \sum_{K \in \P} c_K \, \alpha[K] + \sum_{K \in \P'} c_K \,
        \alpha[K] \\
      &= \sum_{K \in \P} c_K \, \alpha[K] + \sum_{K \in \P'} 0 \times
        \alpha[K] \\
      &=: p.c. \int_{[\P]} f \, d\alpha
    \end{align*}
    as expected.
  \item As in Exercise 11.2.4(h), this is a direct consequence of (a)
    and (g), by defining the function $F : I \to \rr$ and $G : I \to
    \rr$ by
    \[
      \begin{cases}
        F(x) = f|_J(x) & \text{ if } x \in J \\
        F(x) = 0 & \text{ if } x \in I-J
      \end{cases}
    \]
    and
    \[
      \begin{cases}
        G(x) = f|_K(x) & \text{ if } x \in K \\
        G(x) = 0 & \text{ if } x \in I-K
      \end{cases}
    \]
    and using the fact that $f = F+G$ on $I$.
  \end{enumerate}
\end{exo}

\begin{exo}{11.8.4}{State and prove a version of Theorem 11.5.1 for
    the Riemann-Stieltjes integral.}

  This analogue could be stated as follows.

  Let be $I$ a bounded interval, and $X$ a closed interval that
  contains $I$. Let be $f : I \to \rr$ a function that is uniformly
  continuous on $I$, and $\alpha : X \to \rr$ a monotone increasing
  function. Then $f$ is Riemann-Stieltjes integrable on $I$ with
  respect to $\alpha$.

  \medskip
  This theorem can be proved in a similar fashion as Theorem 11.5.1,
  \textit{mutatis mutandis}.

  Since $f$ is uniformly continuous on a bounded interval, then $f$ is
  bounded. We have to prove that $\supint{f}{I} \, d\alpha =
  \infint{f}{I} \, d\alpha$.

  Of course, the statement is clear if $I$ is empty or a singleton. We
  thus suppose that $I$ is an interval of the form $[a,b]$, $]a,b]$,
  $[a,b[$ or $]a,b[$; with $a<b$ two real numbers.

  Let be $\epsilon > 0$ an arbitrary real number. Since $f$ is
  uniformly continuous, there exists a $\delta > 0$ such that $|f(x) -
  f(y)| < \epsilon$ whenever ($x,y \in I$ and) $|x-y| < \delta$.

  In particular, by the Archimedean principle, there exists a natural
  number $N > 0$ such that $(b-a)/N < \delta$. Furthermore, we can
  define a partition $\P$ of $I$ into $N$ intervals,
  $\P := \{J_1, \ldots, J_N\}$, each interval being of length
  $(b-a)/N$.

  Now let be the function $G : I \to \rr$, defined to be piecewise
  constant with respect to $\P$, with constant values
  $c_{J_k} := \sup_{x \in J_k} f(x)$ for all $J_k \in \P$. It is clear
  that $G$ is a piecewise constant function that majorizes $f$ on $I$.
  By definition, we thus have
  \begin{equation*}
    \supint{f}{I} \, d\alpha
    \leq p.c. \int_I G \, d\alpha = \sum_{k=1}^n \sup_{x \in J_k}
    f(x) \alpha[J_k]
  \end{equation*}
  Similarly, we have
  \begin{equation*}
    \infint{f}{I} \, d\alpha
    \geq \sum_{k=1}^N \inf_{x \in J_k} f(x) \alpha[J_k]
  \end{equation*}
  Combining the two previous results, we have
  \begin{equation}
    \label{eq:11.8.4}
    \supint{f}{I} \, d\alpha - \infint{f}{I} \, d\alpha
    \leq \sum_{k=1}^N \Bigl(\sup_{x \in J_k} f(x) - \inf_{x \in J_k}
    f(x)\Bigr)
    \alpha[J_k]
  \end{equation}
  For all $k = 1, \ldots, N$, since $|J_k| = (b-a)/N < \delta$, we have
  $|f(x)-f(y)| < \epsilon$ for all $x,y \in J_k$. Thus, in particular,
  \[f(x) < f(y) + \epsilon \text{ for all } x,y \in J_k\]
  Thus, taking the suprema on $x$, we have
  \[\sup_{x \in J_k} f(x) \leq f(y) + \epsilon\]
  and then taking the infima on $y$,
  \[\sup_{x \in J_k} f(x) - \inf_{y \in J_k} f(y) \leq \epsilon.\]
  Using this bound in equation \eqref{eq:11.8.4} leads to
  \begin{align*}
    \label{eq:11.8.4b}
    \supint{f}{I} \, d\alpha - \infint{f}{I} \, d\alpha
    &\leq \sum_{k=1}^N \epsilon \alpha[J_k] &\\
    &\leq \epsilon \alpha[I] &\text{ (Lemma 11.8.4)}
  \end{align*}
  Since this is true for all $\epsilon > 0$, and since $\alpha[I] \geq
  0$ (because $\alpha$ is supposed to be monotone increasing), it
  shows that $\supint{f}{I} \, d\alpha = \infint{f}{I} \, d\alpha$, as
  expected. This closes the proof.
\end{exo}

\bigskip
\begin{exo}{11.9.2}{Prove Lemma 11.9.5.}

  Let be $I$ a bounded interval, and let $f : I \to \rr$ be a
  function. If $F : I \to \rr$ and $G : I \to \rr$ are two
  antiderivatives of $f$ on $I$, we have to prove that there exists a
  real number $C$ such that $F(x) = G(x) + C$ for all $x \in I$.

  First, note that there is nothing to prove if $I = \emptyset$, and
  that the claim is trivial if $I$ is a singleton. We will thus
  suppose that $I = [a,b]$, $[a,b[$, $]a,b]$ or $]a,b[$ with $a<b$.

  Let be $H = F-G$. By Theorem 10.1.3(f), $H$ is differentiable on
  $I$, and we have $H' = F' - G' = f - f = 0$ on $I$. Thus, in
  particular, for any given limit point $x_0$ of $I$ included in
  $]a,b[$, we have $H'(x_0) = 0$. Now let be an arbitrary $x \in I$.
  \begin{itemize}
  \item If $x > x_0$, then $[x_0, x] \subseteq I$. $H$ is
    differentiable on this closed interval, since it is differentiable
    on $I$. Thus, by the mean value theorem (Corollary 10.2.9), there
    exists $z \in [x, x_0]$ such that $H'(z) := 0 = \frac{H(x_0) -
      H(x)}{x_0 - x}$. Thus, we have $F(x_0) - G(x_0) = F(x) - G(x)$.
    If we set $C := F(x_0) - G(x_0)$, the claim follows.
  \item If $x_0 < x$, then $[x, x_0] \subseteq I$. Thus, the proof is
    similar, just reversing $x$ and $x_0$ when needed.
  \item Finally, if $x=x_0$, we also have $G(x) + C = G(x) + F(x_0) -
    G(x_0) = F(x)$.
  \end{itemize}
\end{exo}

\begin{exo}{11.10.1}{Prove Proposition 11.10.1.}

  We have to prove here the integration by parts formula.
  
  \begin{itemize}
  \item $F'$ is supposed to be Riemann-integrable on $I = [a,b]$.
    Furthermore, $G$ is continuous on $I$ since it is supposed to be
    differentiable on $I$. Thus, by Corollary 11.5.2, $G$ is
    Riemann-integrable on $I$. And then, by Theorem 11.4.5, since $G$
    and $F'$ are Riemann-integrable on $I$, then $F'G$ is
    Riemann-integrable on $I$.
    
    Switching the roles of $F$ and $G$, we can show similarly that
    $FG'$ is Riemann-integrable on $I$.
  \item Also, by Theorem 10.1.3, we know that $FG$ is differentiable,
    and that $(FG)' = F'G + FG'$.
  \item Combining the two previous facts, we have on the one hand:
    \begin{align*}
      \int_I (FG)' &= \int_I (F'G + FG') \\
                   &= \int_I F'G + \int_I FG'
    \end{align*}
    and on the other hand, by the second fundamental theorem of
    calculus (Theorem 11.9.4), we have
    \[
      \int_I (FG)' = F(b)G(b) - F(a)G(a)
    \]
  \item We conclude that $\int_{[a,b]} FG' = F(b)G(b) - F(a)G(a) -
    \int_{[a,b]} F'G$, as expected.
  \end{itemize}
\end{exo}

\begin{exo}{11.10.2}{Fill in the gaps marked (why?) in the proof of
    Lemma 11.10.5.}

  The statements to prove are the following:

  \begin{itemize}
  \item $\phi^{-1}(J)$ is connected.

    Let be $x<y \in \phi^{-1}(J)$. Suppose that $J = [c,d]$ (the fact
    that $J$ is closed or open does not really matter here). By
    Definition 11.1.1, we have to prove that for all $z$ such that
    $x \leq z \leq y$, we have $z \in \phi^{-1}(J)$. Since $\phi$ is
    increasing, we have
    $c \leq \phi(x) \leq \phi(z) \leq \phi(y) \leq d$, which shows
    that $z \in \phi^{-1}(J)$, as expected. Thus, $\phi^{-1}(J)$ is
    connected.

  \item $c_J$ is the constant value of $f \circ \phi$ on
    $\phi^{-1}(J)$.

    Indeed, let be any $x \in \phi^{-1}(J)$. Thus, we have $\phi(x)
    \in J$. But $f$ is supposed to be piecewise constant on $\P$, with
    $c_J$ the constant value of $f$ on $J \in \P$. Thus, $f(\phi(x)) =
    c_J$. But since this is true for any arbitrary $x \in
    \phi^{-1}(J)$, this means that $c_J$ is the constant value of $f
    \circ \phi$ on $\phi^{-1}(J)$.
    
  \item  $\Q := \{\phi^{-1}(J) : J \in \P\}$ is a partition of $[a,b]$.

    First, let be any $x \in [a,b]$. We have $\phi(x) \in [\phi(a),
    \phi(b)]$ because $\phi$ is monotone increasing. Since $\P$ is a
    partition of $[\phi(a), \phi(b)]$, there exists a $J \in \P$ such
    that $\phi(x) \in J$. Thus, there exists at least one $J \in \P$
    such that $x \in \phi^{-1}(J)$, or, in other words, there exists
    at least one $K \in \Q$ such that $x \in K$.

    Now prove that there exists at most one such $K \in \Q$. Suppose
    that there exists $K_1, K_2 \in \Q$, with $K_1 = \phi^{-1}(J_1)$,
    $K_2 = \phi^{-1}(J_2)$, such that we have both $x \in
    K_1, x \in K_2$. We would thus have $\phi(x) \in J_1$ and $\phi(x)
    \in J_2$ at the same time, but this would contradict the fact that
    $\P$ is a partition.

    Thus, for all $x \in [a,b]$, there exists exactly one $K \in \Q$
    such that $x \in K$, which shows that $\Q$ is a partition of
    $[a,b]$.
    
  \item $f \circ \phi$ is piecewise constant with respect to $\Q$.

    Any $K \in \Q$ is of the form $K = \phi^{-1}(J)$, with $J \in \P$.
    We have previously showed that $c_J$ is the constant value of $f
    \circ \phi$ on $\phi^{-1}(J)$; this also shows that $f \circ \phi$
    is piecewise constant with respect to $\Q$.

  \item Finally, $\phi[\phi^{-1}(J)] = |J|$ for all $J \in \P$.

    Let be $J \in \P$. By Definition 11.1.10, $J$ is thus a bounded
    interval; let's call its endpoints $c$ and $d$ (with
    $\phi(a) \leq c < d \leq \phi(b)$). By Definition 11.1.8, we have
    $|J| = d-c$.

    $\phi$ is supposed to be monotone increasing. Thus, if we set
    $d' := \sup\{x \in [a,b] : \phi(x) \leq d\}$ and
    $c' := \inf\{x \in [a,b] : \phi(x) \geq c\}$, then $\phi^{-1}(J)$
    is an interval whose endpoints are $c'$ and $d'$. And then we have
    $\phi[\phi^{-1}(J)] = \phi(d') - \phi(c') = d-c$, as
    expected\footnote{To be picky, some more details could be
      necessary to show that $\phi(d') = d$. Let's denote
      $X := \{x \in [a,b] : \phi(x) \leq d\}$. First, we cannot have
      $\phi(d') < d$, because by the intermediate theorem value
      (Theorem 9.7.1), there would exist a $\xi \in [a, b]$ such that
      $\phi(\xi) = d > \phi(d')$. Because $\phi$ is increasing, we
      would have $\xi > d'$. Since $\xi \in X$, it would be a
      contradiction with the fact that $d'$ is the supremum of $X$.
      Second, we cannot have $\phi(d') > d$ either. By the
      intermediate value theorem, there would exist a $\xi \in [a,b]$
      such that $\phi(\xi) = (\phi(d') + d)/2$, hence
      $\phi(d') > \phi(\xi) > d$. Since $\phi(\xi) < \phi(d')$, we
      would have $\xi < d'$ because $\phi$ is increasing. Meawhile,
      $\xi$ is an upper bound for $X$ : if $x \in X$, then
      $\phi(x) \leq d < \phi(\xi)$. Thus, $\xi$ would be an upper
      bound for $X$ would would be smaller than $d'$, still a
      contradiction with $d'$ being the supremum $X$. Thus, we must
      have $\phi(d') = d$. A similar argument applies for
      $\phi(c') = c$.}
  \end{itemize}
\end{exo}

\begin{exo}{11.10.3}{Let $a<b$ be real numbers, and let $f : [a,b] \to
    \rr$ be a Riemann integrable function. Let $g : [-b,-a] \to \rr$
    be defined by $g(x):=f(-x)$. Show that $g$ is also Riemann
    integrable, and $\int_{[-b,-a]}g = \int_{[a,b]}f$.}

  Since $x \mapsto -x$ is not an increasing function, this is not a
  simple consequence of the results from Section 11.10; we have to
  derive a proper proof instead. We (mainly) have to show that
  $\supint{g}{[-b, -a]} = \infint{g}{[-b, -a]}$.

  \begin{itemize}
  \item First, note that if $\P$ is a partition of $[a,b]$, then $\Q
    := \{ -J : J \in \P\}$ is clearly a partition of $[-b, -a]$ (where
    $-J$ is a shorthand for $\{-x : x \in J\}$).
  \item Let be $\epsilon > 0$ a positive real number. By equation
    \eqref{eq:11.4.1f} (from this document), we know that there exist
    a piecewise constant (with respect to $\P$) function
    $\overline{f}$ that majorizes $f$ on $[a,b]$, and a piecewise
    constant function $\underline{f}$ that minorizes $f$ on $[a,b]$,
    such that
    \begin{equation}
      \label{eq:11.10.3a}
      \int_{[a,b]} f - \epsilon < p.c. \int_{[a,b]} \underline{f} \leq
      \int_{[a,b]} f \leq p.c. \int_{[a,b]} \overline{f} < \int_{[a,b]} f +
      \epsilon.
    \end{equation}  
  \item Note that if we define $\overline{g} : [-b,-a] \to \rr$ by
    $\overline{g}(x) := \overline{f}(-x)$, then $\overline{g}$ is
    clearly a piecewise constant (with respect to $\Q$) function that
    majorizes $g$ on $[-b,-a]$. Furthermore, if $c_J$ is the constant
    value of $\overline{f}$ on $J$, then for all $K := -J \in \Q$, the
    constant value $d_K$ of $\overline{g}$ on $K$ is equal to $c_J$.

    And thus, we have
    \begin{align*}
      p.c. \int_{[-b,-a]} \overline{g}
      &:= \sum_{K \in \Q} d_K |K| \\
      &= \sum_{J \in \P} c_J |-J| \\
      &= \sum_{J \in \P} c_J |J| \\
      &= p.c. \int_{[a,b]} \overline{f}
    \end{align*}
  \item Similarly, if we define $\underline{g} : [-b,-a] \to \rr$ by
    $\underline{g}(x) := \underline{f}(-x)$, then $\underline{g}$ is
    clearly a piecewise constant function that minorizes $g$ on
    $[-b,-a]$; and we have
    \[ p.c. \int_{[-b,-a]} \underline{g} = p.c. \int_{[a,b]}
      \underline{f}\]
  \item Finally, if we come back to equation \eqref{eq:11.10.3a}, we
    have
    \begin{equation*}
      \int_{[a,b]} f - \epsilon < p.c. \int_{[-b,-a]} \underline{g}
      \leq p.c. \int_{[-b,-a]} \overline{g} < \int_{[a,b]} f +
      \epsilon
    \end{equation*}
    and, thus,
    \begin{equation*}
      \int_{[a,b]} f - \epsilon < p.c. \int_{[-b,-a]} \underline{g} \leq
      \infint{g}{[-b, -a]} \leq \supint{g}{[-b, -a]} \leq p.c.
      \int_{[-b,-a]} \overline{g} < \int_{[a,b]} f + \epsilon.
    \end{equation*}
    Since this is true for any arbitrary $\epsilon > 0$, we have
    indeed $\supint{g}{[-b, -a]} = \infint{g}{[-b, -a]}$, so that $g$
    is Riemann-integrable; and we also have $\int_{[-b,-a]} g =
    \int_{[a,b]} f$, as expected.
  \end{itemize}
\end{exo}

\begin{exo}{11.10.4}{What is the analogue of Proposition 11.10.7 when
    $\phi$ is monotone decreasing instead of monotone increasing?}

  This analogue could be stated as follows:
  
  Let $[a,b]$ be a closed interval, and let $\phi : [a,b] \to
  [\phi(b), \phi(a)]$ be a differentiable, monotone decreasing
  function, such that $\phi'$ is Riemann integrable. Let $f: [\phi(b),
  \phi(a)] \to \rr$ be a Riemann integrable function on $[\phi(b),
  \phi(a)]$. Then $(f \circ \phi) \phi' : [a,b] \to \rr$ is Riemann
  integrable on $[a,b]$ and
  
  \[\int_{[a,b]}(f\circ \phi) \phi' = -\int_{[\phi(b),\phi(a)]}f \]

  To prove it, one just have to adapt the proof in Exercise 11.10.3.
\end{exo}
\end{document}
%%% Local Variables:
%%% mode: latex
