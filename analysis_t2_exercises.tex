...
\begin{exo}{12.4.8}{The following construction generalizes the
  ...
  \item The metric space $(\adh{X}, d_{\adh{X}})$ is complete.
    ...
    By definition, $\seq{u_n}{1}$ is a Cauchy sequence of Cauchy
    sequences that take their values in $X$; i.e., for all $k \geq 1$,
    there exists a Cauchy sequence $\seq{x_n^{(k)}}{1}$ of elements of
    $X$ such that $u_k := \formallimit x_n^{(k)}$.
