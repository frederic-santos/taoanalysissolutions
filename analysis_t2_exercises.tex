...
\begin{exo}{12.2.3}{Prove Proposition 12.2.5.}
  ...
  \item First we can form a lemma: for any subset $E \subseteq X$, we
    ...
    Thus, by Definition 12.2.12, we have $E \cap \partial E =
    \emptyset$, so that $\partial E \subseteq X \backslash E$, which
    means that $X \backslash E$ is a closed set.

    The converse also applies: if we suppose that $X \backslash E$ is
    closed, then $\partial (X \backslash E) \subseteq X \backslash E$.
    By equation \eqref{eq:12.2.3} above, this is equivalent to
    $\partial E \subseteq X \backslash E$, and thus we have $\partial
    E \cap E = \emptyset$. This means that $E$ is open, as expected.
  ...
  \item Let $E_1, \ldots, E_n$ be open sets. Thus, for all
    ...
    Thus, by (e), we know that its complementary set,
    $F_1 \cup \ldots \cup F_n$, is closed, as expected.
  \end{enumerate}
\end{exo}
\end{document}
%%% Local Variables:
